...
const updateStatusOnCardDrag = async (card, newStatus) => {
  try {
    await axios.patch(
      `https://projecttracker-pac8.onrender.com/tasks/${card.id}`,
      {
        _id: card.id,
        description: card.description,
        status: newStatus,
...
const deleteTask = async (cardID, onTaskDeleted) => {
  try {
    await axios.delete(
      `https://projecttracker-pac8.onrender.com/tasks/${cardID}`,
      { withCredentials: true }
    );
    if (onTaskDeleted) onTaskDeleted();
  } catch (error) {
...
const fetchTasks = async (setData) => {
  try {
    ...
    const res = await axios.get(
      `https://projecttracker-pac8.onrender.com/tasks/${tokenId}`,
      { withCredentials: true }
    );
    setData(mapTasksToLanes(res.data));
  } catch (error) {
  ...
  const handleDrop = async (e, targetLaneId) => {
    ...
    const updatedLanes = data.lanes.map((lane) => {
      ...
      if (lane.id === targetLaneId) {
        return {
          ...lane,
          cards: [...lane.cards, { ...card, status: newStatus }],
        };
      }
      return lane;
    });
            ...
            {lane.cards.map((card) => (
              ...
              >
                ...
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleEditClick(card, lane.title);
                  }}
                  title="Edit Task"
                >
                  <img src={editIcon} alt="Edit" />
